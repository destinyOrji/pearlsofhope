...
if (!function_exists('validateImageUpload')) {
    function validateImageUpload($file) {
        ...
        // Check MIME type using alternative method
        $mimeType = '';
        if (function_exists('finfo_open')) {
            ...
            // Fallback to mime_content_type or getimagesize
            if (function_exists('mime_content_type')) {
                $mimeType = mime_content_type($file['tmp_name']);
            } else {
                // Use getimagesize as last resort for images
        ...
        if (!empty($mimeType) && !in_array($mimeType, ALLOWED_IMAGE_TYPES)) {
